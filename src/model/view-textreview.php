...
function handleDisplayTextReview() {
    connectToDB();
    global $db_conn;

    $result = executePlainSQL("SELECT * 
    FROM TextReviews TR, Attractions A
    WHERE TR.attractionID = A.attractionID AND TR.reviewID = " . $_GET['reviewID'] . "");

    if (($row = oci_fetch_row($result)) != false) {

        echo "<h2>" . $row[2] . " (ID: " . $row[0] . ")</h2>";
        echo "<h3 style='color: #333;'>Attraction: <a href='view-attraction.php?attractionID="  . $row[8] . "'>"  . $row[9] . "</a> | Rating: " . $row[1] . "/10 | Language: " . $row[5] . "</h3>";
        echo "<table style='width: 100%;'>";
        echo "<tr><td><p style='font-size: 16px; color: #009;'>" . $row[4] . "</p></td></tr>";
